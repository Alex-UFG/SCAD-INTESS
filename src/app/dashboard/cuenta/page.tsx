...
export default async function CuentaPage() {
  ...
  const [roles] = await db.query<RolRow[]>(
    "SELECT nombre FROM rol WHERE id_rol = ? LIMIT 1",
    [session.user.rol]
  );

  const campos = [
    { label: t("nombre"), value: session.user.name ?? "—" },
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-8 sm:px-6">
      ...
      <section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
        ...
        <dl className="mt-4 space-y-3">
          {campos.map((c) => (
            ...
            >
              <dt className="w-40 text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500">
                {c.label}
              </dt>
      ...
      <section className="mt-4 rounded-xl border-2 border-dashed border-slate-300 bg-white/50 p-5 dark:border-slate-700 dark:bg-slate-900/50">
